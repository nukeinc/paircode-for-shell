...
int main()
{
    ...
    //     {0, 0, 1, 1},
    // };
    ystrgetp = readstrfile("../strfilesd1.txt");
    ...
    std::vector<std::vector<int>>rorderrp={rorderp,{0,99,0,1},{0,1,2,4},{0,0,3}};
    rvecall={0,4,8,12,6};
    ...
    mtest=calculateBasisform(5,rorderrp);
    Eigen::MatrixXd m1;
...
// std::cout<<ystrm1[0][1]<<std::endl;
    ...
    std::vector<DataRow> V1val1;
    std::ifstream input_file("../efcsncal.txt"); // 替换为你的文件名
    ...
    std::vector<std::map<int, Matrix4D>>buildVValue2= processFile("../efcsncal.txt");
    ...
    std::vector<double>strength={1.0};
    ...
    std::vector<double> energyp={1.561   ,  0    ,  2.005   ,   0.854   ,   1.363   ,   2.690
   };
    ...
    for (int i=0;i<allbasisp.size();++i)
    {
        ...
        if (i==allbasisp.size()-1)
        {
            ...
            jjv.push_back(allbasisp[i].sj.back()*allbasisp[i].parity);
    ...
    for (size_t num=0;num<blockh.size();++num)
    {
        ...
        for (size_t i = 0; i < eigenvalues1.size(); ++i) {
            outfile << "Eigenvalue " << i + 1 << ": " << eigenvalues1[i] << std::endl;
        }
    ...
    catchpair2=generateValidPairs(4,rorderrp,51);
    ...
    mtest2=calculateBasisform(4,rorderrp);
    Eigen::MatrixXd m2;
    ...
    auto nucmmin1=findMminus1PositionsSTL(nucleusm);

    Eigen::MatrixXd sfmat1=sigsfmat(mtest2, mtest, ystrm2, ystrm1, nucmmin1[4]);
    Eigen::MatrixXd sfchange1=sigsfmatchange(mtest2,allbasisp2, mtest,allbasisp, m2, m1, sfmat1,nucmmin1[4]);
    std::vector<Eigen::MatrixXd> sfmatall;

    for (int i=0;i<nucmmin1.size();++i)
    {
        Eigen::MatrixXd sfmat=sigsfmat(mtest2, mtest, ystrm2, ystrm1, nucmmin1[i]);
        Eigen::MatrixXd sfchange=sigsfmatchange(mtest2,allbasisp2, mtest,allbasisp, m2, m1, sfmat,nucmmin1[i]);
        Eigen::MatrixXd sfmatchsch= schmitmat2*sfchange* schmitmat1.transpose();
        if (!outfile.is_open())
        {
            // 如果文件未打开，则尝试打开文件
            outfile.open("basis_output.txt",std::ios::app);
    ...
    for (int i=0;i<allbasisp2.size();++i)
    {
        ...
        if (i>0)
        {
            if (jvecp1[i]!=jvecp1[i-1] || allbasisp2[i].parity!=allbasisp2[i-1].parity)
            {
                ...
                ii=i;
                jjv1.push_back(allbasisp2[i-1].sj.back()*allbasisp2[i-1].parity);
            }
        }
        if (i==allbasisp2.size()-1)
        {
            blockform11.push_back(i+1-ii);
            jjv1.push_back(allbasisp2[i].sj.back()*allbasisp2[i].parity);
    ...
    for (int y=0;y<sfmatall.size();++y)
    {
        std::vector<std::vector<double> > matrixa2ch=eigenToNestedVector(sfmatall[y]);
            ...
            for (auto& entry1:engre1)
            {
                ...
                for (int i=0;i<values.size();++i)
                {
                    ...
                    for (int j=0;j<values1.size();++j)
                    {
                        ...
                        result += values[i]*values1[j]*matrixa2ch[mnum1][mnum];
                        if ( isnan(result))
                        {
                            std::cout<< matrixa2ch[mnum1][mnum]<<" "<<values[i]<<" "<<values1[j]<<std::endl;
                        }
                    }
                }
                ...
                << " 末态角动量 " << key.first
                << " 末态位置 " << key.second
                << " 谱因子 S " << std::pow(result,2)/(std::abs(key.first)+1)
                << std::endl;
                if (!outfile.is_open())
                {
                    // 如果文件未打开，则尝试打开文件

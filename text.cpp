...
int main()
{
    ...
    Eigen::MatrixXd sfmat1=calsfmat(mtest, mtest2, ystrm1, ystrm2, nucmmin1[4]);
    Eigen::MatrixXd sfchange1=sfmatchange(mtest,allbasisp, mtest2,allbasisp2, m1, m2, sfmat1,nucmmin1[4]);
    std::vector<Eigen::MatrixXd> sfmatall;

    for (int i=0;i<nucmmin1.size();++i)
    {
        Eigen::MatrixXd sfmat=calsfmat(mtest, mtest2, ystrm1, ystrm2, nucmmin1[i]);
        Eigen::MatrixXd sfchange=sfmatchange(mtest,allbasisp, mtest2,allbasisp2, m1, m2, sfmat,nucmmin1[i]);
    ...
    for (int y=0;y<sfmatall.size();++y)
    {
        ...
        for (auto& entry : engre) {
            ...
            for (auto& entry1:engre1)
            {
                ...
                << " 末态角动量 " << key1.first
                << " 末态位置 " << key1.second
                << " 谱因子 S " << std::pow(result,2)/(key.first+1)
                << std::endl;
                if (!outfile.is_open())
                {
                    // 如果文件未打开，则尝试打开文件

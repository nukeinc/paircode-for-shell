...
int main()
{
    ...
    std::vector<double> energyp={1.42644173 ,    -0.16711269   ,   1.75830984   ,   0.54762673  ,    1.05662673  ,    2.57981581};
    ...
    for (int y=0;y<sfmatall.size();++y)
    {
        ...
        for (auto& entry : engre) {
            ...
            for (auto& entry1:engre1)
            {
                ...
                << " 末态角动量 " << key.first
                << " 末态位置 " << key.second
                << " 谱因子 S " << std::pow(result,2)/(key1.first+1)
                << std::endl;
                if (!outfile.is_open())
                {
                    // 如果文件未打开，则尝试打开文件
